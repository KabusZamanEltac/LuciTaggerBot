...
async def help(event):
  ...
  await event.reply(helptext,
                    buttons=(
                      [Button.url('Beni Gruba Ekle➕', 'https://t.me/Werab_tag_botbot?startgroup=a')],
                      [Button.url('Werab Support👨‍💻', 'https://t.me/WerabSupport')],
                      [Button.url('Werab Qoxulu🔖', 'https://t.me/WerabliAnlar')],
		      [Button.url('Sahibi🧑‍🔧', 'https://t.me/ismiyev95')],
                    ),
...
async def mentionall(event):
  ...
  if event.pattern_match.group(1):
    ...
    return await event.respond("Etiketlemek üçün səbəb yaz❗️")
  else:
    return await event.respond("**Etikete Başlamak için səbəb yazın...!**")
  ...
  if mode == "text_on_cmd":
    ...
    async for usr in client.iter_participants(event.chat_id):
      ...
      if event.chat_id not in anlik_calisan:
        await event.respond("** Etiket işlemi başarıyla dayandırıldı❌**")
        return
      if usrnum == 5:
  ...
  if mode == "text_on_reply":
    ...
    async for usr in client.iter_participants(event.chat_id):
      ...
      usrtxt += f"[{random.choice(emoji)}](tg://user?id={usr.id}) "
      if event.chat_id not in anlik_calisan:
        await event.respond("Işlem Başarıyla Durduruldu\n\n**Burada sizində reklamınız ola bilir 👉 @WerabliAnlar**✅")
        return
      if usrnum == 5:
